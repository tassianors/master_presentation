...
\begin{itemize}
  ...
  \item Eles podem ser divididos em dois tipos principais:
\begin{itemize}
...
\begin{itemize}
  \item \textcolor{blue}{Voltera}: Relaciona valores passados da entrada com o valor atual da saída.
  	\begin{equation}
		y(t)=\sum_{j=1}^{\infty}\int_{-\infty}^{\infty}\cdots \int_{-\infty}^{\infty}
		h_j(\tau_1, ... ,\tau_j) \prod_{i=1}^{j}u(t-\tau_i)d\tau_i
	\nonumber
	\end{equation}
  \item \textcolor{blue}{Redes Neurais}: 
  \begin{equation}
  ...
  \end{itemize}
  \item \textcolor{blue}{Funções radiais de base}: Casos particulares de redes neurais, mas lineares nos parâmetros
  $\omega_i$.
  \begin{equation}
...
%-------------------------------------------------------------------------------------------------
\begin{frame}
\frametitle{Enfoque deste trabalho}
%\framesubtitle{Pode ser utilizado para modelos polinômiais}

Neste trabalho serão abordados dois tipos principais de classes de modelos:

\begin{itemize}
	\item Modelo do tipo Wiener
	\begin{itemize}
		\item Subclasse onde a não linearidade estática é um polinómio
	\end{itemize}
	\item Modelo do tipo racional
\end{itemize}

\begin{block}{}
	\begin{itemize}
		\item Para a identificação do modelo Wiener, o método dos mínimos quadrados pode ser utilizado.
		\item Para a identificação do modelo racional o algoritmo seguinte será utilizado: 
	\end{itemize}
	\end{block} 
\end{frame}
...
\framesubtitle{Pode ser utilizado para modelos polinomiais}
...
\column{0.4\textwidth}
\begin{equation}
\psi _n^T(t-1)\theta_n=\sum_{j=1}^{N_n}p_{nj}\theta_{nj}
\nonumber
\end{equation}
\column{0.4\textwidth}
\begin{equation}
\psi _d^T(t-1)\theta_d=\sum_{j=1}^{N_d}p_{dj}\theta_{dj}
\nonumber
\end{equation}
\end{columns}
\end{frame}
