...
\begin{block}{Vantagens VRFT}
\begin{itemize}
  \item Função custo quadrática em $\theta$
  \item Sem problemas de mínimos locais
  \item Não iterativo
\end{itemize}
\end{block}
\end{frame}
...
%-------------------------------------------------------------------------------------------------
\begin{frame}
\frametitle{Classes de modelos abordadas neste trabalho}
%\framesubtitle{União do algoritmo de identificação de sistemas não lineares com referência virtual}


\begin{block}{Classes de modelos exploradas:}
\begin{itemize}
...
\begin{equation}
C(z,\rho) = \rho_1 \frac{1}{z-1} + \rho_2 \frac{z}{z-1}
\nonumber
\end{equation}
...
\column{0.4\textwidth}
100 experimentos Monte Carlo foram realizados e a média das estimativas foi:
...
O custo obtido foi: \textcolor{red}{$J_y(\theta_{\text{média}})=3.0078\times10^{-3}$}
\end{frame}
...
\begin{block}{}
  Quando a planta é um sistema racional e o comportamento em malha fechada é linear, o controlador ideal pode ser
  também representado por um modelo racional.
\end{block}
\vspace{\baselineskip}
...
\begin{block}{Controlador ideal para este conjunto}
\begin{equation}
